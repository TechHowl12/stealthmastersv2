...
const ProductCard = ({ image, title, price, description, setCaseColor, colors }) => {
  ...
  return (
    <div className="rounded-xl bg-[#E6DCC8] h-[12.3rem] sm:h-[21rem] lg:h-[25rem] p-0 border-[#807316] border">
      ...
      <div className="bg-[#ffffffb3] rounded-b-xl px-2 sm:px-3 lg:px-10 py-3 sm:py-6 mt-1 sm:mt-3 lg:mt-8">
        <div className="flex justify-between items-center">
          <h1 className="font-bold text-xs sm:text-xl">{title}</h1>
          <h1 className="font-bold text-xs sm:text-xl text-[#807316]">{price}</h1>
        </div>
        <div className="flex justify-between mt-2 items-center">
          <p className="sm:text-xs text-[7px] font-semibold">{description}</p>
